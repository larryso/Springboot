...
public class OrderController {
	...
	public ModelAndView orderSubmit(HttpServletRequest request, HttpSession session, PaperRecOrderFormBean orderFormBean, Model model) {
		...
		for(String s: productsID) {
			productType = productMap.get(s).getProductCat().getName();
		...
		order.setProduct_type(productType);
		...
		return new ModelAndView("redirect:/my_order");
	}
}
